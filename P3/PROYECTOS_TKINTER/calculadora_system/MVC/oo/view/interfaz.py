...
class vista:
    ...
    def menuprincipal(ventana):
        ...
        OperacionesMenu.add_command(label="Actualizar",command=lambda: vista.actuallizar_dag(ventana))   #vista.actualizar_screen(ventana)
    ...
    def actuallizar_dag(ventana):
        ...
        btngua=Button(ventana,text="Guardar",command=lambda:funciones.funcionesc.enviar_actualizacion(num1.get(),num2.get(),simb.get(),res.get(),id.get()))       
        btngua.pack(pady=5)
        btnvolv=Button(ventana,text="Volver",command=lambda:vista.interfaz(ventana))
